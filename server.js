...
// ✅ Google Login (Passport) - ESM imports
...
// 4) 허용 이메일 목록 검사 함수 (Render 환경변수: ALLOWED_EMAILS)
...
// 5) Google 로그인 전략 (Render 환경변수: GOOGLE_CLIENT_ID / GOOGLE_CLIENT_SECRET)
passport.use(
  new GoogleStrategy(
    {
      clientID: process.env.GOOGLE_CLIENT_ID,
      clientSecret: process.env.GOOGLE_CLIENT_SECRET,
      callbackURL: "https://minimart-dev.onrender.com/auth/google/callback",
    },
...
/* -----------------------------
   ✅ (2) Auth routes (MUST be before requireLogin)
   ----------------------------- */
...
function requireAuth(req, res, next) {
  ...
  if (!sess)
    return res
      .status(401)
      .json({ ok: false, error: "INVALID_SESSION", message: "세션이 만료되었거나 유효하지 않습니다." });
  req.token = token;
  req.session = sess;
  req.db = db;
  next();
...
function requireAdmin(req, res, next) {
  ...
  if (!user || user.role !== "admin")
    return res.status(403).json({ ok: false, error: "FORBIDDEN", message: "관리자 권한이 필요합니다." });
  req.token = token;
  req.session = sess;
  req.user = user;
  req.db = db;
...
app.get("/api/products", (req, res) => {
  ...
  switch (sort) {
    case "price-asc":
      list.sort((a, b) => discountedPrice(a) - discountedPrice(b));
      break;
    case "price-desc":
      list.sort((a, b) => discountedPrice(b) - discountedPrice(a));
      break;
    case "rating":
...
/* ------------------------------------------
   ✅ (4) Protect UI only (most stable)
   - Keep /api/* as-is (no Google login redirect)
   - UI (public) requires Google login
   ------------------------------------------ */

// ✅ UI 보호 시작 (static 바로 위)
